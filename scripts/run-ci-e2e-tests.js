...
const MARKER_TVOS = exec(`mktemp ${ROOT}/local-cli/generator-tvos/templates/app/XXXXXXXX`).stdout.trim();
...
try {
  ...
  if (argv.tvos) {
    echo('Running an tvOS app');
    cd('tvos');
    // Make sure we installed local version of react-native
    if (!test('-e', path.join('EndToEndTest', path.basename(MARKER_TVOS)))) {
      echo('tvOS marker was not found, `react-native init` command failed?');
      exitCode = 1;
      throw Error(exitCode);
    }
    ...
    exec('response=$(curl --write-out %{http_code} --silent --output /dev/null localhost:8081/index.tvos.bundle?platform=tvos&dev=true)');
    echo(`Starting packager server, ${SERVER_PID}`);
    echo('Executing tvos e2e test');
    if (tryExecNTimes(
      () => {
        exec('sleep 10s');
        return exec('xcodebuild -destination "platform=tvOS Simulator,name=Apple TV 1080p,OS=10.0" -scheme EndToEndTest -sdk appletvsimulator test | xcpretty && exit ${PIPESTATUS[0]}').code;
      },
      numberOfRetries)) {
        echo('Failed to run tvOS e2e tests');
  ...
  if (argv.js) {
    ...
    if (exec('react-native bundle --platform tvos --dev true --entry-file index.tvos.js --bundle-output tvos-bundle.js').code) {
      echo('Could not build tvos package');
      exitCode = 1;
      throw Error(exitCode);
    }
